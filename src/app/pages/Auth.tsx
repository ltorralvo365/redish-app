...
import { Leaf, Mail, Lock, Eye, EyeOff, ArrowLeft, ArrowRight, Check } from "lucide-react";
import {
  useAuth,
  AccountType,
  PartnershipType,
...
export function Auth() {
  ...
  function handleAccountType() {
    if (accountType === "consumer") {
      setStep("register-consumer-partnership");
    } else {
      setStep("register-seller");
    }
  }

  function handleConsumerPartnership() {
    if (partnershipType === "personal") {
      setStep("register-consumer-personal");
    } else {
      setStep("register-consumer-commercial");
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-sm">
        ...
        {step === "landing" && (
          <div className="text-center">
            ...
            <button
              ...
              className="w-full bg-green-600 text-white rounded-xl py-3.5 mb-3 shadow hover:bg-green-700 transition-colors"
            >
              Iniciar Sessão
            </button>
            <button
              onClick={() => setStep("register-email")}
              className="w-full bg-white text-green-700 border border-green-200 rounded-xl py-3.5 hover:bg-green-50 transition-colors"
            >
              Criar Conta
            </button>
        ...
        {step === "login" && (
          <div>
            <button onClick={() => back("landing")} className="flex items-center gap-1 text-gray-500 mb-6 hover:text-gray-700">
            ...
            <button onClick={handleLogin} className="w-full bg-green-600 text-white rounded-xl py-3.5 mt-2 shadow hover:bg-green-700 transition-colors">
              Entrar
            </button>
            <p className="text-center text-sm text-gray-500 mt-4">
              Não tens conta?{" "}
              <button onClick={() => back("register-email")} className="text-green-600 underline">
                Criar conta
              </button>
            </p>
          </div>
        ...
        {step === "register-account-type" && (
          <div>
            <button onClick={() => back("register-email")} className="flex items-center gap-1 text-gray-500 mb-6 hover:text-gray-700">
              <ArrowLeft className="w-4 h-4" /> Voltar
            </button>
            ...
            <div className="space-y-3 mb-6">
              <SelectCard
                selected={accountType === "consumer"}
                onClick={() => setAccountType("consumer")}
                emoji="🛒"
                title="Consumidor"
                desc="Quero comprar comida excedente a preços reduzidos."
              />
              <SelectCard
                ...
                emoji="🍽️"
                title="Vendedor"
                desc="Tenho um estabelecimento e quero vender excedentes."
              />
            </div>
        ...
        {/* Consumer — partnership type */}
        {step === "register-consumer-partnership" && (
          <div>
            <button onClick={() => back("register-account-type")} className="flex items-center gap-1 text-gray-500 mb-6 hover:text-gray-700">
              <ArrowLeft className="w-4 h-4" /> Voltar
            </button>
            <StepIndicator current={3} total={3} />
            <h2 className="text-2xl text-gray-900 mb-1" style={{ fontWeight: 700 }}>Tipo de parceria</h2>
            <p className="text-gray-500 text-sm mb-6">Esta conta é para uso pessoal ou organizacional?</p>

            <div className="space-y-3 mb-6">
              <SelectCard
                selected={partnershipType === "personal"}
                onClick={() => setPartnershipType("personal")}
                emoji="👤"
                title="Pessoal"
                desc="Uso individual para poupar dinheiro e reduzir desperdício."
              />
              <SelectCard
                selected={partnershipType === "commercial"}
                onClick={() => setPartnershipType("commercial")}
                emoji="🏢"
                title="Comercial / Organizacional"
                desc="Uma organização que reaproveita ou redistribui excedentes."
              />
            </div>
        ...
        {/* Consumer personal — name */}
        {step === "register-consumer-personal" && (
          <div>
            <button onClick={() => back("register-consumer-partnership")} className="flex items-center gap-1 text-gray-500 mb-6 hover:text-gray-700">
              <ArrowLeft className="w-4 h-4" /> Voltar
            </button>
            <StepIndicator current={3} total={3} />
            ...
            <button
              onClick={() => {
                if (!personalName) { setError("Introduz o teu nome."); return; }
                handleFinalizeConsumer();
              }}
          ...
          <div>
            ...
            <Field label="Nome da organização">
              <input
                ...
                placeholder="Ex: Banco Alimentar Lisboa"
                className="w-full bg-transparent outline-none text-sm text-gray-800 placeholder-gray-400"
              />
            </Field>
            ...
            <button
              onClick={() => {
                if (!orgName) { setError("Introduz o nome da organização."); return; }
                handleFinalizeConsumer();
              }}
              className="w-full bg-green-600 text-white rounded-xl py-3.5 mt-2 flex items-center justify-center gap-2 shadow hover:bg-green-700 transition-colors"
            >
          ...
          <div>
            ...
            <Field label="Localização">
              <input
                ...
                placeholder="Ex: Rua Augusta 10, Lisboa"
                className="w-full bg-transparent outline-none text-sm text-gray-800 placeholder-gray-400"
              />
            </Field>
...
function SelectCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <p className={`text-sm ${selected ? "text-green-700" : "text-gray-800"}`} style={{ fontWeight: 600 }}>
          {title}
        </p>
      ...
      {Array.from({ length: total }).map((_, i) => (
        <div
          ...
          className={`h-1.5 rounded-full transition-all ${
            i + 1 <= current ? "bg-green-500 flex-1" : "bg-gray-200 flex-1"
          }`}
        />
      ))}
    </div>
